...
if not os.path.exists(bootup_file):
    ...
    with open(bootup_file, 'w') as f:
        f.write('')
    xbmc.executebuiltin('Notification(开机默认智能更新CD2和OpenList)')
else:
    # 第二次运行，删除标记文件
    os.remove(bootup_file)
    xbmc.executebuiltin('Notification(开机默认手动更新CD2和OpenList)')
...
if os.path.exists(CE):
   if os.path.exists(CE):
    xbmc.executebuiltin('Notification(开机默认智能更新CD2和OpenList, 设置成功)')

else:
    xbmc.executebuiltin('Notification(开机默认手动更新CD2和OpenList, 设置成功)')
exit()
