...
if not os.path.exists(bootup_file):
    ...
    with open(bootup_file, 'w') as f:
        f.write('')
    xbmc.executebuiltin('Notification(开机默认自动更新CD2和Alist)')
else:
    # 第二次运行，删除标记文件
...
if os.path.exists(nandscript):
    xbmc.executebuiltin('Notification(设置, 开机自动/手动更新挂载插件)')
...
if os.path.exists(CE):
   if os.path.exists(CE):
    xbmc.executebuiltin('Notification(开机默认自动更新CD2和Alist, 设置成功)')
